// functions/api/gebruikers-beheer.js
// Beheer van admin gebruikers: ophalen, aanmaken, verwijderen, reset mail

const AT_BASE    = "appchbjgwoZQiQjfv";
const AT_ADMINS  = "tblxPXaRSgAHiiauP";
const AT_URL     = `https://api.airtable.com/v0/${AT_BASE}/${AT_ADMINS}`;

const CORS = {
  "Access-Control-Allow-Origin":  "*",
  "Access-Control-Allow-Headers": "Content-Type",
  "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
  "Content-Type":                 "application/json",
};

// Sessie verificatie (inline — CF Pages ondersteunt geen module imports)
async function verifieer(token, gebruiker, atToken) {
  try {
    const formule = encodeURIComponent(`{Gebruikersnaam}="${gebruiker}"`);
    const res = await fetch(`${AT_URL}?filterByFormula=${formule}`, {
      headers: { Authorization: `Bearer ${atToken}` }
    });
    if (!res.ok) return false;
    const data  = await res.json();
    const admin = data.records?.[0];
    if (!admin) return false;
    const tok = admin.fields?.ResetToken || "";
    const exp = admin.fields?.ResetVerloopt || "";
    return tok.startsWith("sessie_"+token) && new Date(exp) > new Date();
  } catch { return false; }
}

function randomToken(len=20) {
  const arr = new Uint8Array(len);
  crypto.getRandomValues(arr);
  return Array.from(arr).map(b=>b.toString(16).padStart(2,"0")).join("");
}

export async function onRequest(context) {
  const { request, env } = context;

  if (request.method === "OPTIONS") {
    return new Response("", { status: 200, headers: CORS });
  }

  // Auth
  const url    = new URL(request.url);
  const token  = url.searchParams.get("key");
  const user   = url.searchParams.get("user");

  if (!token || !user) {
    return new Response(JSON.stringify({ error: "Niet geautoriseerd" }), { status: 401, headers: CORS });
  }

  const geldig = await verifieer(token, user, env.AIRTABLE_TOKEN);
  if (!geldig) {
    return new Response(JSON.stringify({ error: "Sessie verlopen" }), { status: 401, headers: CORS });
  }

  // ── GET: alle gebruikers ophalen ──
  if (request.method === "GET") {
    try {
      const res  = await fetch(`${AT_URL}?maxRecords=100`, {
        headers: { Authorization: `Bearer ${env.AIRTABLE_TOKEN}` }
      });
      if (!res.ok) throw new Error("Airtable: "+res.status);
      const data = await res.json();
      const gebruikers = (data.records||[]).map(r=>({
        recordId:        r.id,
        gebruikersnaam:  r.fields?.Gebruikersnaam || "",
        email:           r.fields?.Email || "",
        heeftWachtwoord: !!r.fields?.WachtwoordHash,
      }));
      return new Response(JSON.stringify({ gebruikers }), { status: 200, headers: CORS });
    } catch(e) {
      return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: CORS });
    }
  }

  // ── POST: actie uitvoeren ──
  let body;
  try { body = await request.json(); } catch {
    return new Response(JSON.stringify({ error: "Ongeldige JSON" }), { status: 400, headers: CORS });
  }

  const { actie } = body;

  // Nieuwe gebruiker aanmaken
  if (actie === "aanmaken") {
    const { gebruikersnaam, email } = body;
    if (!gebruikersnaam || !email) {
      return new Response(JSON.stringify({ error: "Gebruikersnaam en e-mail zijn verplicht" }), { status: 400, headers: CORS });
    }

    // Controleer of gebruikersnaam al bestaat
    const check = await fetch(`${AT_URL}?filterByFormula=${encodeURIComponent(`{Gebruikersnaam}="${gebruikersnaam}"`)}`, {
      headers: { Authorization: `Bearer ${env.AIRTABLE_TOKEN}` }
    });
    const checkData = await check.json();
    if (checkData.records?.length > 0) {
      return new Response(JSON.stringify({ error: "Gebruikersnaam bestaat al" }), { status: 400, headers: CORS });
    }

    // Aanmaken in Airtable
    const maakRes = await fetch(AT_URL, {
      method: "POST",
      headers: { Authorization: `Bearer ${env.AIRTABLE_TOKEN}`, "Content-Type": "application/json" },
      body: JSON.stringify({ fields: { Gebruikersnaam: gebruikersnaam, Email: email } })
    });
    if (!maakRes.ok) {
      const err = await maakRes.text();
      return new Response(JSON.stringify({ error: "Airtable: "+err }), { status: 500, headers: CORS });
    }

    // Setup link per mail sturen
    const setupToken = randomToken();
    const setupUrl   = `https://reserveren.l-rijopleidingen.nl/admin.html?setup=${setupToken}&user=${gebruikersnaam}`;

    // Sla setup token op
    const newRecord = await maakRes.json();
    await fetch(`${AT_URL}/${newRecord.id}`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${env.AIRTABLE_TOKEN}`, "Content-Type": "application/json" },
      body: JSON.stringify({ fields: {
        ResetToken:    setupToken,
        ResetVerloopt: new Date(Date.now() + 7*24*60*60*1000).toISOString() // 7 dagen
      }})
    });

    // Verstuur uitnodigingsmail
    await fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: { Authorization: `Bearer ${env.RESEND_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        from:    (env.RESEND_FROM||"").trim(),
        to:      [email],
        subject: "Uitnodiging beheeromgeving — L-Rijopleidingen",
        html:    `<!DOCTYPE html>
<html lang="nl"><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#f5f6f8;font-family:Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 0;">
    <tr><td align="center">
      <table width="480" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;border:1px solid #dde1e9;overflow:hidden;">
        <tr><td style="background:#1a1f2e;padding:20px 32px;">
          <p style="margin:0;color:#fff;font-size:18px;font-weight:700;">L-Rijopleidingen &mdash; Beheer</p>
        </td></tr>
        <tr><td style="padding:28px 32px;">
          <h2 style="font-size:18px;font-weight:700;color:#1a1f2e;margin:0 0 12px">Je bent uitgenodigd</h2>
          <p style="font-size:14px;color:#6b7280;line-height:1.6;margin:0 0 8px">Je hebt toegang gekregen tot de beheeromgeving van L-Rijopleidingen.</p>
          <p style="font-size:14px;color:#6b7280;margin:0 0 20px">Gebruikersnaam: <strong style="color:#1a1f2e">${gebruikersnaam}</strong></p>
          <a href="${setupUrl}" style="display:inline-block;background:#0586f0;color:white;text-decoration:none;padding:12px 24px;border-radius:6px;font-size:14px;font-weight:600;">Wachtwoord instellen</a>
          <p style="font-size:12px;color:#9ca3af;margin:20px 0 0">Deze link is 7 dagen geldig.</p>
        </td></tr>
        <tr><td style="background:#f5f6f8;padding:16px 32px;border-top:1px solid #dde1e9;">
          <p style="margin:0;font-size:12px;color:#9ca3af;">L-Rijopleidingen &middot; info@l-rijopleidingen.nl</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
</body></html>`
      })
    });

    return new Response(JSON.stringify({ success: true }), { status: 200, headers: CORS });
  }

  // Reset mail sturen
  if (actie === "resetmail") {
    const { recordId, email, gebruikersnaam } = body;
    if (!recordId || !email) {
      return new Response(JSON.stringify({ error: "recordId en email verplicht" }), { status: 400, headers: CORS });
    }

    const resetToken = randomToken();
    const verloopt   = new Date(Date.now() + 60*60*1000).toISOString(); // 1 uur

    await fetch(`${AT_URL}/${recordId}`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${env.AIRTABLE_TOKEN}`, "Content-Type": "application/json" },
      body: JSON.stringify({ fields: { ResetToken: resetToken, ResetVerloopt: verloopt } })
    });

    const resetUrl = `https://reserveren.l-rijopleidingen.nl/admin.html?reset=${resetToken}&user=${gebruikersnaam}`;

    await fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: { Authorization: `Bearer ${env.RESEND_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        from:    (env.RESEND_FROM||"").trim(),
        to:      [email],
        subject: "Wachtwoord reset — L-Rijopleidingen Beheer",
        html:    `<!DOCTYPE html>
<html lang="nl"><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#f5f6f8;font-family:Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 0;">
    <tr><td align="center">
      <table width="480" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;border:1px solid #dde1e9;overflow:hidden;">
        <tr><td style="background:#1a1f2e;padding:20px 32px;">
          <p style="margin:0;color:#fff;font-size:18px;font-weight:700;">L-Rijopleidingen &mdash; Beheer</p>
        </td></tr>
        <tr><td style="padding:28px 32px;">
          <h2 style="font-size:18px;font-weight:700;color:#1a1f2e;margin:0 0 12px">Wachtwoord resetten</h2>
          <p style="font-size:14px;color:#6b7280;line-height:1.6;margin:0 0 20px">Klik op de knop hieronder om een nieuw wachtwoord in te stellen.</p>
          <a href="${resetUrl}" style="display:inline-block;background:#0586f0;color:white;text-decoration:none;padding:12px 24px;border-radius:6px;font-size:14px;font-weight:600;">Nieuw wachtwoord instellen</a>
          <p style="font-size:12px;color:#9ca3af;margin:20px 0 0">Deze link is 1 uur geldig.</p>
        </td></tr>
        <tr><td style="background:#f5f6f8;padding:16px 32px;border-top:1px solid #dde1e9;">
          <p style="margin:0;font-size:12px;color:#9ca3af;">L-Rijopleidingen &middot; info@l-rijopleidingen.nl</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
</body></html>`
      })
    });

    return new Response(JSON.stringify({ success: true }), { status: 200, headers: CORS });
  }

  // Gebruiker verwijderen
  if (actie === "verwijderen") {
    const { recordId } = body;
    if (!recordId) {
      return new Response(JSON.stringify({ error: "recordId verplicht" }), { status: 400, headers: CORS });
    }

    const res = await fetch(`${AT_URL}/${recordId}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${env.AIRTABLE_TOKEN}` }
    });

    if (!res.ok) {
      return new Response(JSON.stringify({ error: "Verwijderen mislukt" }), { status: 500, headers: CORS });
    }

    return new Response(JSON.stringify({ success: true }), { status: 200, headers: CORS });
  }

  return new Response(JSON.stringify({ error: "Onbekende actie" }), { status: 400, headers: CORS });
}
