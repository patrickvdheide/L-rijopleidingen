// wijzig-boeking.js — v7
// functions/api/wijzig-boeking.js — v7.0.0
// Wijzigt, annuleert of herstelt een boeking in Airtable en stuurt update-mail
...
export async function onRequest(context) {
  ...
  // Sessie verificatie (inline — geen imports)
  ...
  const _ar = await fetch(
    `https://api.airtable.com/v0/appchbjgwoZQiQjfv/tblxPXaRSgAHiiauP?filterByFormula=${encodeURIComponent('{Gebruikersnaam}="' + user + '"')}`,
  ...
  if (!_rec || !(_rec.fields?.ResetToken || "").startsWith("sessie_" + token) || new Date(_rec.fields?.ResetVerloopt || 0) < new Date()) {
  ...
  if (email) {
    try {
      ...
      const emailHtml = "<!DOCTYPE html><html lang=\"nl\"><head><meta charset=\"UTF-8\"></head><body style=\"margin:0;padding:0;background:#f5f6f8;font-family:Arial,sans-serif;\"><table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" style=\"background:#f5f6f8;padding:32px 0;\"><tr><td align=\"center\"><table width=\"540\" cellpadding=\"0\" cellspacing=\"0\" style=\"background:#fff;border-radius:8px;border:1px solid #dde1e9;\"><tr><td style=\"background:" + kleur + ";padding:24px 32px;\"><p style=\"margin:0;color:#fff;font-size:20px;font-weight:700;\">L-Rijopleidingen</p><p style=\"margin:4px 0 0;color:rgba(255,255,255,0.8);font-size:13px;\">" + koptekst + "</p></td></tr><tr><td style=\"padding:24px 32px;\"><p style=\"font-size:15px;font-weight:600;color:" + kleur + ";margin:0 0 8px;\">" + ikoon + " " + koptekst + "</p><p style=\"font-size:13px;color:#6b7280;margin:0 0 20px;\">Boekingsnummer: <strong>" + boekingsnummer + "</strong></p>" + (annuleer ? "<p style=\"font-size:14px;color:#1a1f2e;line-height:1.6;\">Beste " + naam + ",<br><br>Uw afspraak is geannuleerd door de rijschool.<br><br>Neem contact op via <a href=\"mailto:info@l-rijopleidingen.nl\">info@l-rijopleidingen.nl</a> voor een nieuwe afspraak.</p>" : "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\">" + rijen + "</table>") + "</td></tr><tr><td style=\"background:#f5f6f8;padding:16px 32px;border-top:1px solid #dde1e9;\"><p style=\"margin:0;font-size:12px;color:#9ca3af;\">L-Rijopleidingen · info@l-rijopleidingen.nl</p></td></tr></table></td></tr></table></body></html>";
  ...
  return new Response(
    JSON.stringify({ success: true, actie: annuleer ? "geannuleerd" : herstel ? "hersteld" : "gewijzigd", build: BUILD }),
